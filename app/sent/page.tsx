...
export default function SentPage() {
  ...
  return (
    <main className="space-y-6">
      <h2 className="text-lg font-semibold">Send complete</h2>
            ...
            <tbody>
              {results.map((r: any) => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="p-2 align-top text-red-600">{r.error || ""}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="space-x-2">
        <button onClick={() => router.push("/upload")} className="rounded bg-gray-800 px-3 py-1.5 text-white">Upload more contacts</button>
        <button onClick={() => router.push("/drafts")} className="rounded bg-gray-600 px-3 py-1.5 text-white">Back to drafts</button>
      </div>
    </main>
  );
}
